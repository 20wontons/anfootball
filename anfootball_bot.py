...
```""",
        ...
        embed.set_author(
            ...
            icon_url="https://f4.bcbits.com/img/a2991634193_10.jpg")
        embed.set_footer(text=url)

        await ctx.send("Let's just forget...",
                       embeds=embed,
        ...
        try:
            ...
            button_ctx: interactions.ComponentContext = await bot.wait_for_component(
                components=[nm_display_full_button], timeout=60)
            await button_ctx.edit(content=None,
                                  embeds=tab_embed,
                                  components=[])
        ...
        if e.response.status_code == 404:
            ...
            await ctx.send(f"Unsuccessful connection: {e}. Try again later.",
                           ephemeral=True)
...
@bot.command(
    ...
    options=[
        interactions.Option(name="artist",
                            description="The artist name",
                            type=interactions.OptionType.STRING,
                            required=True),
        interactions.Option(name="song",
        ...
        interactions.Option(
            name="transpose",
            description=
            "The number of semitones to change the tonality of the song",
...
async def chords(ctx, artist: str, song: str, transpose: int = 0):  #url: str
    try:
        ...
        url = UGSearch(json_from_search(
            artist, song)).get_chords_results(1)[0].get_tab_url()
...
async def tabs(ctx, artist: str, song: str):
    try:
        # Takes the first result in the sorted results listing.
        url = UGSearch(json_from_search(
            artist, song)).get_tabs_results(1)[0].get_tab_url()
        ugtabs = UGTab(json_from_url(url))
...
async def search(ctx: interactions.context._Context,
                 sub_command: str,
                 artist: str,
                 song: str = None):
    try:
        if sub_command == "artist":
            ...
            try:
                while True:
                    ...
                    button_ctx: interactions.ComponentContext = await bot.wait_for_component(
                        components=[
                            search_left_button, search_choose_button,
                            search_right_button
                        ],
                        timeout=60)
                    ...
                    await button_ctx.edit(embeds=results_embeds[page],
                                          components=search_row)
            except asyncio.TimeoutError:
                return await ctx.edit(embeds=results_embeds[page],
                                      components=[])

            url = "https://www.ultimate-guitar.com" + results[
                page].get_artist_url()
        ...
        button_ctx = await bot.wait_for_component(components=[close_button],
                                                  timeout=None)
...
@bot.command(
    ...
    options=[
        interactions.Option(name="today",
                            description="Today's most popular tabs.",
                            type=interactions.OptionType.SUB_COMMAND),
        interactions.Option(name="popular",
...
def _format_tab_embed(ugtab: UGTab) -> interactions.Embed:
    ...
    embed = interactions.Embed(title=ugtab.get_artist() + " - " +
                               ugtab.get_song() + " (" + ugtab.get_type() +
                               ")",
...
def _format_results_embeds(
        results: "list[UGSearchResult | UGArtist]"
...
search_right_button = interactions.Button(
    style=interactions.ButtonStyle.PRIMARY, label="Next", custom_id="next")
search_left_button = interactions.Button(
    style=interactions.ButtonStyle.PRIMARY, label="Prev", custom_id="prev")
...
link_button = interactions.Button(style=interactions.ButtonStyle.LINK,
                                  label="Open in UG")
close_button = interactions.Button(style=interactions.ButtonStyle.DANGER,
                                   label="Close",
                                   custom_id="close")
tab_row = interactions.ActionRow(components=[link_button, close_button])
