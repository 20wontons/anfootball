...
```""",
        ...
        await ctx.send(
            "This is not a valid `tabs.ultimate-guitar.com/tabs/` link.",
            ephemeral=True)
...
async def chords(ctx, artist: str, song: str, transpose: int = 0):  #url: str
    try:
        ...
        url = UGSearch(json_from_search(
            artist, song)).get_chords_results()[0].get_tab_url()
...
async def tabs(ctx, artist: str, song: str):
    try:
        # Takes the first result in the sorted results listing.
        url = UGSearch(json_from_search(
            artist, song)).get_tabs_results()[0].get_tab_url()
        ugtabs = UGTab(json_from_url(url))
...
async def search(ctx: interactions.context._Context, sub_command: str,
                 artist: str, song: str):
    try:
        ...
        if sub_command == "all":
            results = ugsearch.get_chords_results(
            )[:5] + ugsearch.get_tabs_results()[:5]
...
def _format_results_embeds(results):
